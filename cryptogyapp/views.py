...
def index(request):
    ...
    context = {"cryptosystem_list": cryptosystem_list}
...
def rsaView(request):
    ...
    if request.is_ajax and request.method == "POST":
        ...
        if form.is_valid():
            ...
            if cleartextParam != "":
                ...
                try:
                    ciphertextParam = [
                        int(x.strip()) for x in ciphertextParam.split(" ")
                    ]
                    try:
                        ...
                        return JsonResponse({"error": "Hubo un error."}, status=200)
                    return JsonResponse(
                        {"cleartext": cleartext, "pParam": pParam, "qParam": qParam},
                        status=200,
                    )
                except Exception as e:
                    print("Error:", e)
    ...
    context = {"thisCryptosystem": thisCryptosystem, "form": form}
    return HttpResponse(template.render(context, request))
            ...
            if cleartextParam != "":
                ...
                try:
                    ...
                    for ciphtext in ciphertextParam:
                        ...
                        cltext = int(
                            format(
                                rabin.decryption(ciphtext, pParam, qParam), "x"
                            ).zfill(226 // 4),
                            16,
                        )
                        cltext = cltext.to_bytes(
                            ((pParam * qParam).bit_length() + 7) // 8, "big"
                        ).decode("utf-8", "strict")
    ...
    if request.is_ajax and request.method == "POST":
        ...
        if form.is_valid():
            ...
            if pubKey != "" and privKey != "":
                ...
                alpha, k = menezesvanstone.generate_keys(
                    params, generator, cleartextParam
                )
            ...
            if cleartextParam != "":
                ...
                print("Encriptado.")
                try:
                    ciphertext = menezesvanstone.encrypt(
                        cleartextParam, alpha, k, params, generator
                    )
                    newCipherText = ""
                    ...
                    return JsonResponse(
                        {"ciphertext": newCipherText, "alpha": alpha, "k": k},
                        status=200,
                    )
                except Exception as e:
                    print("Error:", e)
                ...
                try:
                    ...
                    return JsonResponse({"error": "Hubo un error."}, status=200)
                return JsonResponse(
                    {"cleartext": cleartext, "alpha": alpha, "k": k}, status=200
                )
    ...
    if request.is_ajax and request.method == "POST":
        ...
        if form.is_valid():
            ...
            if message != "" and signature != "":
                ...
                return JsonResponse(
                    {"isValid": isValid, "pParam": str(pParam), "qParam": str(qParam)},
                    status=200,
                )
    ...
    if request.is_ajax and request.method == "POST":
        ...
        if form.is_valid():
            ...
            if message != "" and signature != "":
                ...
                return JsonResponse(
                    {"isValid": isValid, "pubkey": str(B), "privkey": str(b)},
                    status=200,
                )
                ...
                return JsonResponse(
                    {"signature": new_signature, "pubkey": str(B), "privkey": str(b)},
                    status=200,
                )
...
def imageEncryption(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # process the data in form.cleaned_data as required
            if (
                form.cleaned_data["clearImage"] != None
                and form.cleaned_data["cipherImageT1"] == None
                and form.cleaned_data["cipherImageT2"] == None
            ):
                # try:
                ...
                return render(
                    request,
                    "imageencryption.html",
                    {
                        "form": form,
                        "img_obj": img_obj,
                        "img_t1": "/media/images/temp_img_t1.png",
                        "img_t2": "/media/images/temp_img_t2.png",
                    },
                )
                # except:
            ...
            if (
                ...
                # try:
                vsss.decrypt_image(
                    settings.BASE_DIR + img_obj.cipherImageT1.url,
                    settings.BASE_DIR + img_obj.cipherImageT2.url,
                )
                # except:
                #     return render(request, 'imageencryption.html', {'form': form})
                return render(
                    ...
                    "imageencryption.html",
                    {
                        "form": form,
                        "img_obj": img_obj,
                        "clear_image": "/media/images/clear_image.png",
                    },
                )
        else:
            print("Invalid form.")
...
def blockchainSimulation(request):
    if request.method == "POST":
        ...
        if "validateblockchain" in request.POST:
            print("Validar Blockchain")
            if blockchain.isValid():
                print("El blockchain ha sido validado.")
                thisCryptosystem = Cryptosystem.objects.get(
                    name="Blockchain Simulation"
                )
                template = loader.get_template("cryptogyapp/blockchain-validado.html")
                context = {
                    "thisCryptosystem": thisCryptosystem,
                }
                return HttpResponse(template.render(context, request))
            else:
                print("El blockchain no ha sido validado.")
                thisCryptosystem = Cryptosystem.objects.get(
                    name="Blockchain Simulation"
                )
                template = loader.get_template(
                    "cryptogyapp/blockchain-no-validado.html"
                )
                context = {
                    "thisCryptosystem": thisCryptosystem,
                }
        ...
        elif "corruptblockchain" in request.POST:
            print("La blockchain ha sido dañada.")
            if (blockchain.chain):
            ...
            template = loader.get_template("cryptogyapp/blockchain-danado.html")
            context = {
                "thisCryptosystem": thisCryptosystem,
            }
            return HttpResponse(template.render(context, request))
            ...
            if form.is_valid():
                ...
                if num == 0:
                    ...
                    initial_block_obj = Block(
                        number=initial_block.number,
                        hash=initial_block.hash(),
                        previous_hash=initial_block.previous_hash,
                        data=initial_block.data,
                        nonce=initial_block.nonce,
                        numoftransactions=0,
                    )
